...
public class GameLoop
  extends Thread
  {
    ...
    private float[] calibratedAccelValues = new float[3];  // Calibration values so device can be held however
    ...
    public GameLoop(TwinTiltCode mainThread, Context passedContext, GLRenderer glRenderer, int maxWidth, int maxHeight)
      {
        ...
        calibratedAccelValues[0] = 0;  // initialize acceleration values to 0
        calibratedAccelValues[1] = 0;  // as above
        calibratedAccelValues[2] = 0;  // as above
    ...
    public void SetAccelValues(float valueX, float valueY, float valueZ)
      {
        ...
        if (( totalAccel != 0 ) && ( isNan == false ))  // make sure we can divide correctly
          {
            accelValues[0] = (float) Math.asin(valueY / totalAccel) +
                             calibratedAccelValues[0];  // from accelration to tilt, X
            accelValues[1] = -(float) Math.asin(valueX / totalAccel) +
                             calibratedAccelValues[1];    // from accelration to tilt, Y
            accelValues[2] = (float) Math.asin(valueZ / totalAccel) +
                             calibratedAccelValues[2];  // from accelration to tilt, Z
          }
      }
