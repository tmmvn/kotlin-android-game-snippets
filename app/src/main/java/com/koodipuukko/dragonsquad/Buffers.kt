...
class Buffers {
	...
	fun changeTextureBuffer(index: Int, value: Float) {
		textureCoordinates[index] = value
		if(DEBUG) {
			Log.d(
				TAG,
				"Buffers: texture buffer at index $index changed to $value"
			)
		}
	}
	// Same as above
	...
	fun clearBuffers(i: Boolean, v: Boolean, t: Boolean) {
		if(v) {
			vbb?.clear()
			vertexBuffer.clear()
			try {
	...
	fun createBuffers(i: Boolean, v: Boolean, t: Boolean) {
		clearBuffers(i, v, t)
		if(i) {
			createIndexBuffer()
		}
