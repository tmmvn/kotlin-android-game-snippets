...
class SoundManager
	...
	fun playMusic(trackId: Int) {
		...
		if(trackId == 2) {
			try {  // parse the music uri, catch and log any errors
				bgMusic =
					Uri.parse("android.resource://jeweltilt.craneam.com/") //add track name here + R.raw.);
		...
		{
			Log.d(TAG, "Music volume set at: $newVolume")
		}
	...
	fun playSound(index: Int) {
		...
		val streamVolume: Int =
			mAudioManager!!.getStreamVolume(AudioManager.STREAM_MUSIC)
		val newVolume = (streamVolume / mAudioManager.getStreamMaxVolume
			(AudioManager.STREAM_MUSIC)).toFloat()
		if(DEBUG) {
	...
	companion object {
		...
		private const val NUM_SOUNDS = 0 // Constant for number of sounds
	}
}
