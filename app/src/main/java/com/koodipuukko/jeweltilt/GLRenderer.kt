...
class GLRenderer
	...
	private val playerRenderList: MutableList<PlayerBall?> =
		ArrayList<PlayerBall?>()
	private val jewelRenderList: MutableList<Jewel?> = ArrayList<Jewel?>()
	private val uiRenderList: MutableList<UIElement?> = ArrayList<UIElement?>()
	private val sparkleRenderList: MutableList<Sparkle?> = ArrayList<Sparkle?>()
		...
		while(syncing) {
			if(!renderingPlayer && !clearPlayer) {
				...
				for(i in listToCopy.indices) {
					...
					try {
						playerRenderList.set(i, listToCopy[i])
						if(DEBUG) {
							Log.d(
								TAG,
		...
		while(syncing) {
			if(!renderingSparkle && !clearSparkle) {
				...
				for(i in listToCopy.indices) {
					...
					try {
						sparkleRenderList.set(i, listToCopy[i])
						if(DEBUG) {
							Log.d(
								TAG,
		...
		while(syncing) {
			if(!renderingJewels && !clearJewels) {
				...
				for(i in listToCopy.indices) {
					...
					try {
						jewelRenderList.set(i, listToCopy[i])
						if(DEBUG) {
							Log.d(
								TAG,
		...
		while(syncing) {
			if(!renderingUI && !clearUI) {
				...
				for(i in listToCopy.indices) {
					...
					try {
						uiRenderList.set(i, listToCopy[i])
						if(DEBUG) {
							Log.d(
								TAG,
